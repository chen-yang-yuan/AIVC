...
here::i_am("code/2_outcome/plot.R")
dpi <- 500
...
# All genes
genes <- read.csv(here::here(paste0("data/", data, "/processed_data/genes.csv")))
genes <- genes$genes

# SG markers
sg_markers_df <- read_excel(here::here("data/utils/SG_markers.xlsx"))
...
t.test(df[df$is_SG_marker == "SG markers", ]$in_cytoplasm_ratio, df[df$is_SG_marker == "Non SG markers", ]$in_cytoplasm_ratio)
